...
cleanup() {
  if [[ -d "${staging_dir}" ]]; then
    mv "${staging_dir}" "/tmp/lolobuysheet-lt-pages-staging-${$}" 2>/dev/null || true
  fi
}
trap cleanup EXIT
...
if [[ -f "${staging_dir}/_worker.js/wrangler.json" ]]; then
  mv \
    ...
    "/tmp/lolobuysheet-lt-generated-wrangler-${$}.json"
fi
...
mv \
  ...
  "/tmp/lolobuysheet-lt-worker-modules-${$}"
mv "${staging_dir}/_worker.bundle.js" "${staging_dir}/_worker.js"

if [[ -f "${deploy_redirect}" ]]; then
  mv \
    "${deploy_redirect}" \
    "/tmp/lolobuysheet-lt-wrangler-deploy-redirect-${$}.json"
fi

backup_dir="/tmp/lolobuysheet-lt-vinext-dist-${$}"
