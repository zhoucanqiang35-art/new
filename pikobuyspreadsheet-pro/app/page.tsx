...
export const metadata: Metadata = {
  ...
  description: "Research PikoBuy finds by category, compare visible product evidence, review QC photos, and plan shipping and returns with fact-checked guides.",
...
const homeSchema = {
  ...
  "@graph": [
    {
      "@type": "WebSite",
      "@id": "https://pikobuyspreadsheet.pro/#website",
      ...
      potentialAction: {
        "@type": "SearchAction",
        target: "https://findspreadsheet.com/search.html?channelid=2&method=1&keywords={search_term_string}",
    ...
    },
    {
      ...
      description: "An independently maintained directory of product research records, category checklists, QC guidance, shipping guidance and source-review notes.",
      url: "https://pikobuyspreadsheet.pro/",
      isAccessibleForFree: true,
      keywords: ["PikoBuy spreadsheet", "PikoBuy finds", "QC photos", "shipping research", "product links"],
      creator: { "@id": "https://pikobuyspreadsheet.pro/#website" },
      hasPart: { "@id": "https://pikobuyspreadsheet.pro/#categories" },
    },
    {
      "@type": "ItemList",
      "@id": "https://pikobuyspreadsheet.pro/#categories",
      name: "PikoBuy product research categories",
      numberOfItems: categories.length,
      itemListElement: categories.map(([name, , href], index) => ({
        "@type": "ListItem",
        position: index + 1,
        name,
        url: new URL(href, "https://pikobuyspreadsheet.pro").toString(),
      })),
    },
    {
      "@type": "FAQPage",
...
export default function Home() {
  return <main className="v3" id="top">
    ...
    <MobileFold title="Interactive row checker"><section className="v3-section v3-wrap" id="score">
      ...
      <div className="v3-tool-layout"><ResearchScore/><aside><span>WHY THIS MATTERS</span><h3>Seven visible checks turn an endless spreadsheet into a defensible shortlist.</h3><p>Begin with category fit. Then inspect photos, sizing and the original source. Price becomes meaningful only beside comparable rows and likely shipping weight.</p><ul><li><b>6–7</b> Strong shortlist candidate</li><li><b>4–5</b> Research more</li><li><b>2–3</b> Weak row</li><li><b>0–1</b> Remove for now</li></ul><Link href="/en/qc-photos">Open the QC field guide →</Link></aside></div>
    </section></MobileFold>
    ...
    <MobileFold title="Published order flow"><section className="v3-process"><div className="v3-wrap">
      ...
      <div className="v3-citation"><span>SOURCE RECHECKED 11 SEP 2026</span><p>Process wording was reviewed against the platform’s published materials. Account-specific status belongs to the platform support channel.</p><Link href="/en/sources">Read methodology →</Link></div>
    </div></section></MobileFold>
    ...
    <MobileFold title="Independent decision pages"><section className="v3-guides" id="guides"><div className="v3-wrap">
      ...
      <div className="v3-guide-grid">{guideDefs.map((guide, index) => <Link href={`/en/${guide.slug}`} key={guide.slug}><span>{String(index + 1).padStart(2, "0")}</span><small>{guide.kicker}</small><h3>{guide.title}</h3><p>{guide.summary}</p><b>Read guide →</b></Link>)}</div>
    </div></section></MobileFold>
    ...
    <MobileFold title="Research sources"><section className="v3-ledger"><div className="v3-wrap"><div><span>RESEARCH SOURCE LEDGER</span><h2>Facts readers can review.</h2><p>The methodology page records the public materials checked for process and policy statements without sending visitors away from this site.</p></div><div><Link href="/en/how-to-use"><span>01</span><b>Platform overview</b><small>Discovery, purchasing, inspection and shipping</small><strong>→</strong></Link><Link href="/en/guides"><span>02</span><b>Beginner workflow</b><small>Decision points across the order process</small><strong>→</strong></Link><Link href="/en/shipping"><span>03</span><b>Shipping research</b><small>Destination, type, weight and dimensions</small><strong>→</strong></Link><Link href="/en/returns"><span>04</span><b>Returns research</b><small>Eligibility, timing and possible fees</small><strong>→</strong></Link></div></div></section></MobileFold>

    <footer className="v3-footer"><div className="v3-wrap"><Brand /><p>Independent informational website built to support FindSpreadsheet product research. It does not place orders, process payments, control parcels or guarantee listings.</p><div><a href="https://findspreadsheet.com/" target="_blank" rel="noreferrer">FindSpreadsheet ↗</a><Link href="/en/faq">FAQ</Link><a href="#top">Back to top ↑</a></div></div></footer>
  </main>;
}
