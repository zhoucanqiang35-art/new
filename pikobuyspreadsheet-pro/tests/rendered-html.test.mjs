...
test("renders production indexing metadata and structured data", async () => {
  ...
  assert.match(html, indexFollowMeta);
  assert.ok(!html.includes('name="codex-preview"'), "production output must not include preview metadata");
  assert.ok(html.includes('"@type":"Dataset"'), "homepage should describe the research dataset");
  assert.ok(html.includes('"@type":"ItemList"'), "homepage should describe the category list");
});

test("renders route-specific SEO schema and direct canonical links", async () => {
  const worker = await loadWorker();
  const cases = [
    ["/en/product-categories", "CollectionPage"],
    ["/en/product-details", "ItemList"],
  ...
  for (const [path, schemaType] of cases) {
    ...
    const html = await response.text();
    assert.ok(html.includes(`\"@type\":\"${schemaType}\"`), `${path} should include ${schemaType} schema`);
    assert.ok(html.includes('"@type":"BreadcrumbList"'), `${path} should include breadcrumb schema`);
    assert.ok(html.includes(`rel="canonical" href="https://pikobuyspreadsheet.pro${path}"`), `${path} should use its 200 URL as canonical`);
  }
});
