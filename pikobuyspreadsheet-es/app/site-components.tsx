...
export function SeoArticleGrid({ locale = "en-US" }: { locale?: string }) {
  ...
  return <div className="seo-grid">{seoArticles.map((article,i) => { const shippingTopic=["shipping","tracking","restricted"].some((part)=>article.slug.includes(part)); const countryName=article.slug.includes("spain")?"España":article.slug.includes("usa")?"USA":article.slug.includes("uk-guide")?"UK":article.slug.includes("germany")?"Deutschland":""; const countryTopic=Boolean(countryName); const topic = countryTopic ? `${t.navCountries} · ${countryName}` : shippingTopic ? t.navShipping : article.slug.includes("qc") ? `${e.productDetails} · QC` : t.workflow; return <Link key={article.slug} href={localPath(locale,`seo-articles/${article.slug}`)}><span>{e.seoArticles} {String(i+1).padStart(2,"0")}{translated ? "" : ` · ${article.readTime}`}</span><h3>{translated ? `PikoBuy — ${topic}` : article.title}</h3><p>{translated ? (countryTopic||shippingTopic ? e.researchIntro : article.slug.includes("qc") ? e.productsIntro : e.workflowIntro) : article.excerpt}</p><b>{e.readMore} →</b></Link>;})}</div>;
}
