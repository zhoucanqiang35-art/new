...
export function HomePage({ locale = "en-US" }: { locale?: string }) {
  ...
  const homepageFaqSchema={"@context":"https://schema.org","@type":"FAQPage",mainEntity:localizedFaq.slice(0,6).map((item)=>({"@type":"Question",name:item.question,acceptedAnswer:{"@type":"Answer",text:item.answer}}))};
  const quickCategoryIndexes=[1,2,4,8] as const;
  return <PageFrame locale={locale}>
    ...
    <section className="hero"><div className="hero-copy"><span className="eyebrow">{t.eyebrow}</span><h1>{t.title}</h1><p>{t.intro}</p><div className="hero-search"><SearchForm buttonLabel={e.searchButton} placeholder={t.searchPlaceholder} emptyMessage={`${t.search}: ${t.searchPlaceholder}`}/></div><div className="hero-categories" aria-label={e.categories}>{quickCategoryIndexes.map((index)=><a key={categories[index].slug} href={`${mainSite}${categories[index].path}`} target="_blank" rel="noopener noreferrer">{l.categories[index]}</a>)}</div><div className="actions"><a className="button" href={`${mainSite}/AllProducts/`} target="_blank" rel="noopener">{t.browse} ↗</a><Link className="button button-secondary" href={localPath(locale,"guides")}>{t.navGuides} →</Link><Link className="text-link" href={localPath(locale,"methodology")}>{t.evidence} →</Link></div></div><div className="hero-panel"><MobileFold label={l.researchSnapshot} openLabel={l.open} closeLabel={l.close}><div className="sticker">{e.independent}</div><span className="section-index">{e.primarySources} / 2026</span><div className="metrics"><div><strong>241</strong><span>{t.navCountries}</span></div><div><strong>47</strong><span>{e.categories}</span></div><div><strong>13</strong><span>{t.navShipping}</span></div><div><strong>6</strong><span>{t.workflow}</span></div></div><p>{e.researchIntro}</p></MobileFold></div></section>
...
export function ProductDetailPage({ slug,locale="en-US" }: { slug:string;locale?:string }) {
  const product=products.find((item)=>item.slug===slug); if(!product) notFound(); const t=getCopy(locale),e=getExtra(locale),translated=!isEnglishLocale(locale);
