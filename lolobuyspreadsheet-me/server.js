...
const seoCatalog = {
  category: {
    shoes: { label: "Shoes", summary: "A category research page for comparing source links, option details, photos, measurements and parcel-weight context before opening the main database.", checks: ["Side profile, outsole and heel photos", "Exact size option and insole measurement", "Material and colour details", "Packed-weight context for the parcel"] },
    hoodies: { label: "Hoodies", summary: "A category research page for checking garment measurements, fabric details and selected options before a product is saved.", checks: ["Chest and length in centimetres", "Fabric weight, cuffs and hood details", "Print or embroidery placement", "Selected colour and size"] },
    "t-shirts": { label: "T-Shirts", summary: "A category research page for comparing a current source listing with fit, fabric and print evidence.", checks: ["Chest and length measurements", "Fabric and seam details", "Print placement or finish", "Current option and visible price"] },
    jackets: { label: "Jackets", summary: "A category research page for research on shell, lining, closures, dimensions and parcel volume.", checks: ["Shell and lining details", "Chest, length and sleeve measurements", "Closure and pocket photos", "Likely parcel volume"] },
    "pants-shorts": { label: "Pants & Shorts", summary: "A category research page for comparing fit measurements and material details against the live product source.", checks: ["Waist, rise and inseam", "Leg opening and fabric details", "Wash and colour", "Selected size"] },
    headwear: { label: "Headwear", summary: "A category research page for checking dimensions, adjustment method and close-up construction evidence.", checks: ["Internal circumference or adjustment", "Logo or patch close-up", "Material and shape", "Selected colour"] },
    sets: { label: "Sets", summary: "A category research page for matching every included piece, size option and parcel context.", checks: ["Included pieces and quantities", "Top and bottom measurements", "Matching colourway", "Combined parcel context"] },
    underwear: { label: "Underwear", summary: "A category research page for reviewing material, size-chart and pack-count evidence.", checks: ["Material information", "Size-chart measurements", "Pack count", "Selected option"] },
    jersey: { label: "Jersey", summary: "A category research page for comparing size, name/number option and fabric information.", checks: ["Chest and length measurements", "Selected name or number option", "Fabric and badge details", "Current source match"] },
    accessories: { label: "Accessories", summary: "A category research page for recording dimensions, material, hardware and the selected variant.", checks: ["Scale and dimensions", "Material and hardware", "Selected variant", "Packaging context"] }
  },
  brand: {
    nike: { label: "Nike", summary: "A brand research hub for locating category-relevant source links and documenting the exact option before further research." },
    stussy: { label: "Stussy", summary: "A brand research hub for locating category-relevant source links and documenting the exact option before further research." }
  },
  batch: {
    gx: { label: "GX", summary: "A research tag page. A batch label is only a seller or community description; it is not independent verification of materials, origin, authenticity or quality." },
    "pk-4-0": { label: "PK 4.0", summary: "A research tag page. Recheck the live source and category-specific photos; a batch label alone does not establish product quality or availability." }
  }
};

function breadcrumb(lang, items) {
  return '<nav class="crumbs" aria-label="Breadcrumb" style="display:flex;gap:7px;flex-wrap:wrap;margin:0 0 26px;font-size:13px">' + items.map(function(item, index) {
    return index === items.length - 1 ? '<span aria-current="page">' + esc(item[0]) + '</span>' : '<a href="' + item[1] + '?lang=' + lang + '">' + esc(item[0]) + '</a><span aria-hidden="true">/</span>';
  }).join('') + '</nav>';
}

function detailLink(path, lang, label) {
  return '<a class="button" href="' + path + '?lang=' + lang + '">' + label + '</a>';
}

function categoryIndex(lang) {
  const cards = Object.entries(seoCatalog.category).map(function(entry) {
    const slug = entry[0], data = entry[1];
    return '<a href="/category/' + slug + '?lang=' + lang + '"><p class="eyebrow">Category research</p><h2>' + data.label + '</h2><p>' + data.summary + '</p><b>Open category →</b></a>';
  }).join('');
  return page(lang, "Category directory", "LoloBuy category research pages", "Choose a category to see a consistent evidence checklist before you search the live database.", breadcrumb(lang, [["Home", "/"], ["Category", "/category"]]) + '<section class="article-list">' + cards + '</section>');
}

function catalogIndex(lang, type) {
  const title = type === "brand" ? "LoloBuy brand research pages" : "LoloBuy batch research pages";
  const cards = Object.entries(seoCatalog[type]).map(function(entry) {
    const slug = entry[0], data = entry[1];
    return '<a href="/' + type + '/' + slug + '?lang=' + lang + '"><p class="eyebrow">' + (type === "brand" ? "Brand research" : "Research tag") + '</p><h2>' + data.label + '</h2><p>' + data.summary + '</p><b>Open page →</b></a>';
  }).join('');
  return page(lang, type === "brand" ? "Brand directory" : "Batch directory", title, "These indexes organise research topics; they do not verify sellers, inventory, product quality or shipping availability.", breadcrumb(lang, [["Home", "/"], [type === "brand" ? "Brand" : "Batch", "/" + type]]) + '<section class="article-list">' + cards + '</section>');
}

function relatedResearch(lang, type, slug) {
  const categoryCards = Object.entries(seoCatalog.category).slice(0, 4).map(function(entry) {
    return '<a href="/category/' + entry[0] + '?lang=' + lang + '"><h3>' + entry[1].label + '</h3><p>Use the category checklist before opening live results.</p><b>Open category →</b></a>';
  }).join('');
  return '<section><p class="eyebrow">Related research</p><h2>Continue with a clear question.</h2><div class="guide-grid">' + categoryCards + '</div></section>';
}

function catalogDetail(lang, type, slug) {
  const data = seoCatalog[type][slug];
  if (!data) return type === "category" ? categoryIndex(lang) : catalogIndex(lang, type);
  const typeLabel = type === "category" ? "Category" : type === "brand" ? "Brand" : "Batch";
  const checks = data.checks || ["Open the live source and save the exact option", "Compare category-relevant photos and measurements", "Record what is missing or still uncertain", "Use the current database for availability"];
  const list = '<ul class="actions">' + checks.map(function(check) { return '<li>' + esc(check) + '</li>'; }).join('') + '</ul>';
  const body = breadcrumb(lang, [["Home", "/"], [typeLabel, "/" + type], [data.label, "/" + type + "/" + slug]]) +
    '<section class="article"><h2>What to check for ' + esc(data.label) + '</h2><p>' + esc(data.summary) + '</p>' + list + detailLink("https://findspreadsheet.com/AllProducts/", lang, "Search the current database →") + '</section>' +
    relatedResearch(lang, type, slug) +
    '<section class="scope"><h2>Research boundary</h2><p>This page is an independent checklist, not a product listing or a claim of seller verification, authenticity, availability, final price or delivery outcome.</p></section>';
  return page(lang, typeLabel + " research", data.label + " LoloBuy spreadsheet research", data.summary, body);
}

function itemDetail(lang, slug) {
  const items = {
    "dunk-vt": { label: "Dunk Low VT", category: "Shoes", categorySlug: "shoes" }
  };
  const item = items[slug];
  if (!item) return productDetails(lang);
  const body = breadcrumb(lang, [["Home", "/"], ["Category", "/category"], [item.category, "/category/" + item.categorySlug], [item.label, "/item/" + slug]]) +
    '<section class="article"><h2>Evidence checklist for this research entry</h2><p>This template is designed to turn one saved product reference into a reviewable record. It does not make a statement about brand ownership, authenticity, seller reliability, stock, pricing or delivery.</p><ul class="actions"><li>Match the live product source to the saved title and option.</li><li>Save category-specific photos and measurements.</li><li>Record warehouse weight only when it is available.</li><li>Use the live database before making a purchasing decision.</li></ul>' + detailLink("https://findspreadsheet.com/AllProducts/", lang, "Check current product data →") + '</section>' + relatedResearch(lang, "item", slug);
  return page(lang, "Item research entry", item.label + " research checklist", "A structured review page for source matching, visible evidence and shipment planning.", body);
}

function documentMeta(path) {
  const clean = path.replace(/\/$/, "") || "/";
  if (clean === "/") return { title: "LoloBuy Spreadsheet 2026 | Research, QC & Product Links", description: "Independent LoloBuy spreadsheet research: compare live product sources, category-specific evidence, measurements and parcel context." };
  if (clean === "/category") return { title: "LoloBuy Categories | Research Checklists", description: "Browse category-specific LoloBuy spreadsheet research checklists for source matching, photos, measurements and parcel context." };
  if (clean.startsWith("/category/")) { const data = seoCatalog.category[clean.split("/").pop()]; return { title: data ? "LoloBuy " + data.label + " Spreadsheet | Research Checklist" : "LoloBuy Category Research", description: data ? data.summary : "Category-specific LoloBuy spreadsheet research." }; }
  if (clean === "/brand") return { title: "LoloBuy Brand Research Pages", description: "Organised brand research pages for documenting source links and options before live database research." };
  if (clean.startsWith("/brand/")) { const data = seoCatalog.brand[clean.split("/").pop()]; return { title: data ? "LoloBuy " + data.label + " Research Page" : "LoloBuy Brand Research", description: data ? data.summary : "Independent brand research page." }; }
  if (clean === "/batch") return { title: "LoloBuy Batch Research Pages", description: "Research-tag pages for documenting labels and checking live evidence without unsupported product claims." };
  if (clean.startsWith("/batch/")) { const data = seoCatalog.batch[clean.split("/").pop()]; return { title: data ? "LoloBuy " + data.label + " Batch Research" : "LoloBuy Batch Research", description: data ? data.summary : "Independent batch research page." }; }
  if (clean.startsWith("/item/")) return { title: "LoloBuy Item Research Checklist", description: "Structured source, photo, measurement and parcel-context checklist for a saved product research entry." };
  return { title: pageTitles.en, description: "Independent LoloBuy spreadsheet research for clearer product, QC and parcel decisions." };
}

function structuredData(path) {
  const clean = path.replace(/\/$/, "") || "/";
  const parts = clean.split("/").filter(Boolean);
  const breadcrumbItems = [{ "@type": "ListItem", position: 1, name: "Home", item: originUrl("/") }].concat(parts.map(function(part, index) {
    return { "@type": "ListItem", position: index + 2, name: part.replace(/-/g, " "), item: originUrl("/" + parts.slice(0, index + 1).join("/")) };
  }));
  const graphs = [{ "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: breadcrumbItems }];
  if (clean === "/category/shoes") graphs.push({ "@context": "https://schema.org", "@type": "ItemList", name: "LoloBuy Shoes Research Index", description: "Independent research checklist for shoes.", itemListElement: [{ "@type": "ListItem", position: 1, name: "Dunk Low VT research entry", url: originUrl("/item/dunk-vt") }] });
  return '<script type="application/ld+json">' + JSON.stringify(graphs).replace(/<\//g, "<\\/") + '</script>';
}
function originUrl(path) { return "https://lolobuyspreadsheet.me" + path; }
  ...
  async fetch(req) {
    const url = new URL(req.url),
      lang = locales[url.searchParams.get("lang")]
        ...
        : "en",
      path = url.pathname;
    const meta = documentMeta(path);
    let body;
    if (path === "/") body = home(lang);
    else if (path === "/category") body = categoryIndex(lang);
    else if (path.startsWith("/category/")) body = catalogDetail(lang, "category", path.split("/").pop());
    else if (path === "/brand") body = catalogIndex(lang, "brand");
    else if (path.startsWith("/brand/")) body = catalogDetail(lang, "brand", path.split("/").pop());
    else if (path === "/batch") body = catalogIndex(lang, "batch");
    else if (path.startsWith("/batch/")) body = catalogDetail(lang, "batch", path.split("/").pop());
    else if (path.startsWith("/item/")) body = itemDetail(lang, path.split("/").pop());
