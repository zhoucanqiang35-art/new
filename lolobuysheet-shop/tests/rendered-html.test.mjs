...
test("publishes localized core pages and English-only long-form articles", async () => {
  ...
  assert.equal((sitemapXml.match(/<url>/g) ?? []).length, 464);
  ...
  assert.match(sitemapXml, /https:\/\/lolobuysheet\.shop\/seo-articles\/lolobuy-payment-methods-account-balance/);
...
test("publishes the English-only payment methods article with current evidence", async () => {
  const workerUrl = new URL("../dist/server/index.js", import.meta.url);
  workerUrl.searchParams.set("payment-article-test", `${process.pid}-${Date.now()}`);
  const { default: worker } = await import(workerUrl.href);
  const env = { ASSETS: { fetch: async () => new Response("Not found", { status: 404 }) } };
  const ctx = { waitUntil() {}, passThroughOnException() {} };

  const response = await worker.fetch(new Request("http://localhost/seo-articles/lolobuy-payment-methods-account-balance", { headers: { accept: "text/html" } }), env, ctx);
  const html = await response.text();
  assert.equal(response.status, 200);
  assert.match(html, /LoloBuy Payment Methods: Checkout, Balance and Refund Decisions/);
  assert.match(html, /Official sources and verification record/);
  assert.match(html, /https:\/\/www\.lolobuy\.com\/noticeDetail\?id=679635137462928/);
  assert.match(html, /"@type":"Article"/);
  assert.match(html, /"@type":"BreadcrumbList"/);
  assert.match(html, /"datePublished":"2026-08-14"/);
  assert.match(html, /"dateModified":"2026-08-14"/);
  assert.match(html, /rel="canonical" href="https:\/\/lolobuysheet\.shop\/seo-articles\/lolobuy-payment-methods-account-balance"/);
  assert.match(html, /href="https:\/\/www\.lolobuy\.com\/noticeDetail\?id=1242596425270255"/);
  assert.match(html, /href="\/seo-articles\/lolobuy-total-cost-fees-checklist"/);
  assert.match(html, /LoloBuy spreadsheet product-discovery hub/);

  const untranslated = await worker.fetch(new Request("http://localhost/de/seo-articles/lolobuy-payment-methods-account-balance", { headers: { accept: "text/html" } }), env, ctx);
  assert.equal(untranslated.status, 404);
});
