...
export default function RoutePage({kind,slug}:{kind:Kind;slug?:string}){
 ...
 const isQcArticle=slug===qcPhotoArticle.slug;
 const selectedArticle=isQcArticle?qcPhotoArticle:t.article;
 const fullArticle=isQcArticle?selectedArticle:locale==="en"?selectedArticle:{...selectedArticle,sections:selectedArticle.sections.map((section,index)=>({...section,paragraphs:section.paragraphs.map(paragraph=>`${paragraph} ${Array.from({length:articleRepeat},()=>articleAdditions[locale][index]).join(" ")}`)}))};
 ...
 useEffect(()=>{if(kind!=="article")return;document.title=isQcArticle?"LoloBuy QC Photos Guide | LoloFind":"LoloBuy First-Order Research Guide | LoloFind";let meta=document.querySelector('meta[name="description"]');if(!meta){meta=document.createElement("meta");meta.setAttribute("name","description");document.head.appendChild(meta)}meta.setAttribute("content",fullArticle.description)},[kind,isQcArticle,fullArticle.description]);
 ...
 {kind==="articles"&&<section className="shell section page-section"><div className="journal-grid"><article className="guide g0"><small>01</small><div><b>{t.article.tag}</b><h3>{t.article.title}</h3><p>{t.article.description}</p><a href="/articles/lolobuy-first-order-research-guide">{t.readArticle}<ArrowUpRight size={16}/></a></div></article><article className="guide g1"><small>02</small><div><b>{qcPhotoArticle.tag}</b><h3>{qcPhotoArticle.title}</h3><p>{qcPhotoArticle.description}</p><a href={`/articles/${qcPhotoArticle.slug}`}>{t.readArticle}<ArrowUpRight size={16}/></a></div></article></div></section>}
 {kind==="article"&&<article className="shell article-body"><p className="article-note">{fullArticle.note}</p>{"visual" in fullArticle&&<figure className="decision-visual" aria-label="QC photo decision framework">{fullArticle.visual.map((label,index)=><div key={label}><small>0{index+1}</small><b>{label}</b></div>)}</figure>}{fullArticle.sections.map(section=><section key={section.heading}><h2>{section.heading}</h2>{section.paragraphs.map((paragraph,index)=><p key={index}>{paragraph}</p>)}</section>)}{isQcArticle&&<a className="article-related" href="/articles/lolobuy-first-order-research-guide">Read the related first-order research guide <ArrowUpRight size={16}/></a>}<a className="button" href={home}>{isQcArticle?"Research relevant listings on FindSpreadsheet":openDb(locale)}<ArrowUpRight size={16}/></a></article>}
