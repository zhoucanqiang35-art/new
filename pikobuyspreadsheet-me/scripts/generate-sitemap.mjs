import { writeFileSync } from "node:fs";

const base = "https://pikobuyspreadsheet.me";
const output = process.argv[2] || "public/sitemap.xml";
const locales = ["de", "fr", "es", "it", "pt", "nl", "pl"];
const categories = ["shoes", "t-shirts", "hoodies-sweaters", "jackets", "pants-shorts", "bags", "accessories", "electronics"];
const pages = [
  { path: "", lastmod: "2026-08-29", changefreq: "weekly", priority: "1.0" },
  { path: "product-details", lastmod: "2026-08-29", changefreq: "monthly", priority: "0.8" },
  { path: "how-pikobuy-works", lastmod: "2026-08-29", changefreq: "monthly", priority: "0.8" },
  { path: "qc-guide", lastmod: "2026-08-29", changefreq: "monthly", priority: "0.8" },
  { path: "shipping-guide", lastmod: "2026-08-29", changefreq: "monthly", priority: "0.8" },
  { path: "buyer-checklist", lastmod: "2026-08-29", changefreq: "monthly", priority: "0.8" },
  { path: "faq", lastmod: "2026-08-29", changefreq: "monthly", priority: "0.8" },
  { path: "seo-articles", lastmod: "2026-09-26", changefreq: "weekly", priority: "0.9" },
];

const urls = [];
for (const page of pages) urls.push({ ...page, url: `${base}/${page.path}` });
urls.push({ path: "how-to-use-pikobuy-spreadsheet", url: `${base}/how-to-use-pikobuy-spreadsheet`, lastmod: "2026-08-29", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-return-policy", url: `${base}/pikobuy-return-policy`, lastmod: "2026-08-31", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-product-links", url: `${base}/pikobuy-product-links`, lastmod: "2026-09-02", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-shipping-calculator", url: `${base}/pikobuy-shipping-calculator`, lastmod: "2026-09-04", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-payment-methods", url: `${base}/pikobuy-payment-methods`, lastmod: "2026-09-22", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-forwarding-guide", url: `${base}/pikobuy-forwarding-guide`, lastmod: "2026-09-08", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-tracking", url: `${base}/pikobuy-tracking`, lastmod: "2026-09-10", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-customs", url: `${base}/pikobuy-customs`, lastmod: "2026-09-12", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-packaging", url: `${base}/pikobuy-packaging`, lastmod: "2026-09-14", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-warehouse", url: `${base}/pikobuy-warehouse`, lastmod: "2026-09-16", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-order-status", url: `${base}/pikobuy-order-status`, lastmod: "2026-09-18", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-parcel-submission", url: `${base}/pikobuy-parcel-submission`, lastmod: "2026-09-20", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-shipping-times", url: `${base}/pikobuy-shipping-times`, lastmod: "2026-09-22", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-parcel-consolidation", url: `${base}/pikobuy-parcel-consolidation`, lastmod: "2026-09-24", changefreq: "monthly", priority: "0.9" });
urls.push({ path: "pikobuy-shipping-insurance", url: `${base}/pikobuy-shipping-insurance`, lastmod: "2026-09-26", changefreq: "monthly", priority: "0.9" });
for (const locale of locales) for (const page of pages) urls.push({ ...page, url: `${base}/${locale}${page.path ? `/${page.path}` : ""}`, priority: page.path ? page.priority : "0.8" });
for (const locale of ["", ...locales]) for (const category of categories) urls.push({ url: `${base}${locale ? `/${locale}` : ""}/categories/${category}`, lastmod: "2026-08-29", changefreq: "monthly", priority: "0.7" });

const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls.map(item => `  <url>\n    <loc>${item.url}</loc>\n    <lastmod>${item.lastmod}</lastmod>\n    <changefreq>${item.changefreq}</changefreq>\n    <priority>${item.priority}</priority>\n  </url>`).join("\n")}\n</urlset>\n`;
writeFileSync(output, xml);
console.log(`Wrote ${urls.length} canonical URLs to ${output}`);
