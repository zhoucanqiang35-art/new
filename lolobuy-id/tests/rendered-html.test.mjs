...
test("publishes the warehouse storage guide only at its English canonical URL", async () => {
  const workerUrl = new URL("../dist/server/index.js", import.meta.url);
  workerUrl.searchParams.set("storage-test", `${process.pid}-${Date.now()}`);
  const { default: worker } = await import(workerUrl.href);
  const env = { ASSETS: { fetch: async () => new Response("Not found", { status: 404 }) } };
  const context = { waitUntil() {}, passThroughOnException() {} };
  const slug = "lolobuy-warehouse-storage-period-guide";

  const articleResponse = await worker.fetch(new Request(`https://lolobuy.id/en/article/${slug}`), env, context);
  const articleHtml = await articleResponse.text();
  assert.equal(articleResponse.status, 200);
  assert.match(articleHtml, /<title>LoloBuy Warehouse Storage: Period, Fees &amp; Deadlines<\/title>/);
  assert.match(articleHtml, new RegExp(`rel="canonical" href="https://lolobuy\\.id/en/article/${slug}"`));
  assert.match(articleHtml, new RegExp(`hrefLang="en" href="https://lolobuy\\.id/en/article/${slug}"`));
  assert.doesNotMatch(articleHtml, new RegExp(`/de/article/${slug}`));
  assert.match(articleHtml, /"@type":"Article"/);
  assert.match(articleHtml, /Read the published 180-day rule carefully/);
  assert.match(articleHtml, /https:\/\/findspreadsheet\.com\//);
...
test("publishes the payment guide only at its English canonical URL", async () => {
  const workerUrl = new URL("../dist/server/index.js", import.meta.url);
  workerUrl.searchParams.set("payment-test", `${process.pid}-${Date.now()}`);
  const { default: worker } = await import(workerUrl.href);
  const env = { ASSETS: { fetch: async () => new Response("Not found", { status: 404 }) } };
  const context = { waitUntil() {}, passThroughOnException() {} };
  const slug = "lolobuy-payment-methods-balance-guide";

  const articleResponse = await worker.fetch(new Request(`https://lolobuy.id/en/article/${slug}`), env, context);
  const articleHtml = await articleResponse.text();
  assert.equal(articleResponse.status, 200);
  assert.match(articleHtml, /<title>LoloBuy Payment Methods, Balances &amp; Parcel Adjustments<\/title>/);
  assert.match(articleHtml, new RegExp(`rel="canonical" href="https://lolobuy\\.id/en/article/${slug}"`));
  assert.match(articleHtml, new RegExp(`hrefLang="en" href="https://lolobuy\\.id/en/article/${slug}"`));
  assert.doesNotMatch(articleHtml, new RegExp(`/de/article/${slug}`));
  assert.match(articleHtml, /"@type":"Article"/);
  assert.match(articleHtml, /Map the payment to the order stage/);
  assert.match(articleHtml, /https:\/\/findspreadsheet\.com\//);
...
test("publishes the parcel packaging guide only at its English canonical URL", async () => {
  const workerUrl = new URL("../dist/server/index.js", import.meta.url);
  workerUrl.searchParams.set("packaging-test", `${process.pid}-${Date.now()}`);
  const { default: worker } = await import(workerUrl.href);
  const env = { ASSETS: { fetch: async () => new Response("Not found", { status: 404 }) } };
  const context = { waitUntil() {}, passThroughOnException() {} };
  const slug = "lolobuy-parcel-packaging-removal-guide";

  const articleResponse = await worker.fetch(new Request(`https://lolobuy.id/en/article/${slug}`), env, context);
  const articleHtml = await articleResponse.text();
  assert.equal(articleResponse.status, 200);
  assert.match(articleHtml, /<title>LoloBuy Parcel Packaging: Box Removal &amp; Protection<\/title>/);
  assert.match(articleHtml, new RegExp(`rel="canonical" href="https://lolobuy\\.id/en/article/${slug}"`));
  assert.match(articleHtml, new RegExp(`hrefLang="en" href="https://lolobuy\\.id/en/article/${slug}"`));
  assert.doesNotMatch(articleHtml, new RegExp(`/de/article/${slug}`));
  assert.match(articleHtml, /"@type":"Article"/);
  assert.match(articleHtml, /Close the product decision before changing its packaging/);
  assert.match(articleHtml, /https:\/\/findspreadsheet\.com\/shoes\//);
...
test("publishes the forwarding service guide only at its English canonical URL", async () => {
  const workerUrl = new URL("../dist/server/index.js", import.meta.url);
  workerUrl.searchParams.set("forwarding-test", `${process.pid}-${Date.now()}`);
  const { default: worker } = await import(workerUrl.href);
  const env = { ASSETS: { fetch: async () => new Response("Not found", { status: 404 }) } };
  const context = { waitUntil() {}, passThroughOnException() {} };
  const slug = "lolobuy-forwarding-service-warehouse-guide";

  const articleResponse = await worker.fetch(new Request(`https://lolobuy.id/en/article/${slug}`), env, context);
  const articleHtml = await articleResponse.text();
  assert.equal(articleResponse.status, 200);
  assert.match(articleHtml, /<title>LoloBuy Forwarding Service: Warehouse &amp; Inspection<\/title>/);
  assert.match(articleHtml, new RegExp(`rel="canonical" href="https://lolobuy\\.id/en/article/${slug}"`));
  assert.match(articleHtml, new RegExp(`hrefLang="en" href="https://lolobuy\\.id/en/article/${slug}"`));
  assert.doesNotMatch(articleHtml, new RegExp(`/de/article/${slug}`));
  assert.match(articleHtml, /"@type":"Article"/);
  assert.match(articleHtml, /Copy the warehouse address from the authenticated page/);
  assert.match(articleHtml, /https:\/\/findspreadsheet\.com\//);
...
test("publishes the lost and damaged parcel guide only at its English canonical URL", async () => {
  const workerUrl = new URL("../dist/server/index.js", import.meta.url);
  workerUrl.searchParams.set("parcel-claim-test", `${process.pid}-${Date.now()}`);
  const { default: worker } = await import(workerUrl.href);
  const env = { ASSETS: { fetch: async () => new Response("Not found", { status: 404 }) } };
  const context = { waitUntil() {}, passThroughOnException() {} };
  const slug = "lolobuy-lost-damaged-parcel-claim-guide";

  const articleResponse = await worker.fetch(new Request(`https://lolobuy.id/en/article/${slug}`), env, context);
  const articleHtml = await articleResponse.text();
  assert.equal(articleResponse.status, 200);
  assert.match(articleHtml, /<title>LoloBuy Lost Package: Damage Claims &amp; Evidence<\/title>/);
  assert.match(articleHtml, new RegExp(`rel="canonical" href="https://lolobuy\\.id/en/article/${slug}"`));
  assert.match(articleHtml, new RegExp(`hrefLang="en" href="https://lolobuy\\.id/en/article/${slug}"`));
  assert.doesNotMatch(articleHtml, new RegExp(`/de/article/${slug}`));
  assert.match(articleHtml, /"@type":"Article"/);
  assert.match(articleHtml, /Use the shortest published deadline, not the most generous one/);
  assert.match(articleHtml, /https:\/\/findspreadsheet\.com\//);
  ...
  for (const locale of ["de", "fr", "es", "it", "pt"]) {
    const untranslatedResponse = await worker.fetch(new Request(`https://lolobuy.id/${locale}/article/${slug}`), env, context);
    assert.equal(untranslatedResponse.status, 404);
  }
  ...
  assert.equal(sitemapResponse.status, 200);
  assert.match(sitemapXml, new RegExp(`<loc>https://lolobuy\\.id/en/article/${slug}</loc>`));
  assert.match(sitemapXml, /<lastmod>2026-09-25T00:00:00\.000Z<\/lastmod>/);
  assert.doesNotMatch(sitemapXml, new RegExp(`<loc>https://lolobuy\\.id/de/article/${slug}</loc>`));
});
