...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{const {slug}=await params;const article=articles.find(a=>a.slug===slug);return article?{title:`${article.title} | FindSpreadsheet Agent Guide`,description:article.excerpt,alternates:{canonical:`/article/${slug}`}}:{};}
